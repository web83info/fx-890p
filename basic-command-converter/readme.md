...
機械語で組まれています。まずは機械語エリアを確保します。

```
CLEAR 1024,2022,6144
``` 
 
次に，リスト1を入力します。見かけはBASICプログラムですが，自作のMTOOLを使ってコンバ－トされているので，中身は機械語プログラムです。
...
```
 ...
 と入力して機械語を書き込みます。そうしたら2000Hから26DDHまでを保存しましょう。保存する方法は，FDDなら，
 
 ```
 DEFSEG=0
 BSAVE"0:BCC.BIN",&H2000,&H6DE
 ```
 
 となります。これでこのツ－ルの使用が可能となります。
 
 リスト1は普通のチェックサムの計算方法と違うので，モニタでは打ち込めません。ご了承ください。
...
これはキャラクタ－コ－ドを表示するプログラムです。これが[P2]エリアに入っていると仮定しましょう。機械語プログラムがメモリ上にあることを確認し，
